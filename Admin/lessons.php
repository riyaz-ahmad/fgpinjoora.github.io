  ...
  while($row = $result->fetch_assoc()){
    if(isset($_REQUEST['checkid']) && $_REQUEST['checkid'] == $row['course_id']){
      ...
      if(($row['course_id']) == $_REQUEST['checkid']){ 
        ...
        ?>
        <h3 class="mt-5 bg-dark text-white p-2">Service ID : <?php if(isset($row['course_id'])) {echo $row['course_id']; } ?> Service Name: <?php if(isset($row['course_name'])) {echo $row['course_name']; } ?></h3>
        <?php
          ...
          echo '<table class="table">
            <thead>
              <tr>
              <th scope="col">Sub-Services ID</th>
              <th scope="col">Sub-Services Name</th>
              <th scope="col">Sub-Services Link</th>
